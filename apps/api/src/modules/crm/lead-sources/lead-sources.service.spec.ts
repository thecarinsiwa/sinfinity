...
describe('LeadSourcesService', () => {
  ...
  const row = {
    id: '0191e6b8-4c3a-7b2d-9f1e-srcsrsrsrsrs',
  ...
  beforeEach(() => {
    ...
    service = new LeadSourcesService(db as never);
  });

  it('lists sources for org', async () => {
    db.select
    ...
    expect(result.data[0].code).toBe('LINKEDIN');
  });
  ...
  it('maps duplicate code to ConflictException', async () => {
    ...
    await expect(
      service.create({ code: 'LINKEDIN', name: 'LinkedIn' }, orgId, orgUser),
