...
describe('swagger-tags', () => {
  it('defines Phase 0–13 tags in stable order', () => {
  ...
  it('uses the Achats tag name expected by ROADMAP', () => {
    expect(SWAGGER_TAG.Achats).toBe('Achats');
  });
  ...
  it('uses the Stock tag name expected by ROADMAP', () => {
    expect(SWAGGER_TAG.Stock).toBe('Stock');
  });
